...
export const resolutionsSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder.addCase(resolutions.success, (state, { payload }) => {
      state.resolutions = [...payload.resolutions];
      state.status = 'success';
    });

    builder.addCase(resolutions.pending, (state) => {
      state.status = 'loading';
    });
    builder.addCase(resolutions.failed, (state) => {
      state.status = 'failed';
    });
  },
});
...
export const selectResolutionsArray = createSelector(
  getResolutionsState,
